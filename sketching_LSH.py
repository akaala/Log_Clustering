...
class SketchString():
    ...
    def displayWordVector(self,delimiter=", "):
        length=len(self.Words_Vector)
        print  delimiter.join(self.Words_Vector[i] for i in range(length))
    ...
    def Diff_bits_signature(self,b1):
        XOR=(self.bloom_vector ^ b1)
            #print "Bloom-filter", self.bloom_vector ^ b1,"(XOR)",
        return XOR,XOR.count()

    def Common_bits_signature(self,b1):
        AND=(self.bloom_vector & b1)
            #print "Bloom-filter", self.bloom_vector & b1,"(AND)",
        return AND,AND.count()

    def Imp_bits_signature(self,b1):
        OR=(self.bloom_vector | b1)
        #print "Bloom-filter", self.bloom_vector | b1 ,"(OR)",
        return OR,OR.count()
...
def Comp_log():
    ...
    for file in all_files:
        ...
        if fp:
            ...
            for line in iter_file:
                if line.strip():
                    Log_all.append((((line.replace(",", "")).rstrip('\n')).replace(")", " ")) .replace("(", " "))
                    #Log_all.append((line.replace(",", "")).rstrip('\n'))
                    Hashed.append(0)
                    #print line
                    #print Hashed[findex]
    ...
    one_to_many=0
    Log_Sig=[]
                        #Log_Sig=[bitarray(MAX_BITS) for i in xrange(10)]
    ...
    for i in range(log_len):
        ...
        if 1:
            ...
            #print "i--->",i,"Sig_len--->",Sig_len
            if Sig_len==0:
                    Log_Sig.append(ns.bloom_vector)
                    #print Log_Sig
                    Hashed[i]=1
            else :
                for j in range(Sig_len):
                    #print "j--->",j,"Sig_len--->",Sig_len
                    Common_sig,Common_sig_count=ns.Common_bits_signature(Log_Sig[j])
                    Diff_sig,Diff_sig_count=ns.Diff_bits_signature(Log_Sig[j])
                    Imp_sig,Imp_sig_count=ns.Imp_bits_signature(Log_Sig[j])
                    Pattern_count=Log_Sig[j].count()
                    Per_Diff_sig=Diff_sig_count*1.0/Imp_sig_count
                    Per_Common_sig=Common_sig_count*1.0/Imp_sig_count
                        #print Common_sig
                    if (Per_Common_sig>PER_MATCH):
                        Log_Sig[j]=Common_sig
                        is_match = True
                        one_to_many=one_to_many+1
                        if (Hashed_Per<Per_Common_sig):
                            Hashed_Per=Per_Common_sig
                            Hashed[i]=j+1
                            
                        
                        #if one_to_many>1 && (hashed_count<Per_Common_sig)
                            
                            
        
                        #print "I",i,"$J",j,"$",Log_Sig[j].count(),"$",h1,"$",Common_sig_count,"$",Diff_sig_count,"$",Per_Common_sig,"$",Per_Diff_sig
                        #print j,Per_Common_sig
                if not is_match:
                    Log_Sig.append(ns.bloom_vector)
                    #Hashed[i]=j
        Sig_len=len(Log_Sig)
